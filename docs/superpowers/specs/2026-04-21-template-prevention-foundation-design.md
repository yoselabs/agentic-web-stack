...
- ~~Grit vs script for `perspective-boundary` and `scoped-landmark-queries`.~~ **Decided: both ship as `scripts/check-*.ts`, not Grit plugins.** The two rules needed capabilities Biome 2.4.12's embedded Grit engine does not expose cleanly: (1) *file-path-based exemption* (Grit patterns match syntax, not path context — emulating "exempt if this file is listed in a JSON config" would require a side-channel unavailable in the plugin ABI); (2) *preceding-line comment lookup* (Grit's multi-line context is single-node-scoped; matching "the `// placement-agnostic:` comment on the line above" requires statement-index arithmetic the AST matcher doesn't expose). Both were already anticipated as fallback candidates in the spec body, so shipping the scripts is on-path. The scripts live at `scripts/check-perspective-boundary.ts` (config-driven via `.perspective-boundary.json`, empty on this template) and `scripts/check-scoped-landmarks.ts` (ratcheted via `.scoped-landmarks-allow.json` for pre-existing step files). Both have fixture tests and narrow turbo inputs identical to the other `scripts/check-*.ts` family members — no net new infra.
