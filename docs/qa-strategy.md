...
- `lint:check:<name>` (14 and counting) — custom `scripts/check-*.ts` enforcing repo-specific invariants
...
3. **What's its place in the pyramid?** If slower than #5, it belongs at on-demand / post-deploy tier. If faster than #2, it's probably a static check — add a `scripts/check-*.ts`.
