...
- [x] **Close `.config/allowlists/test-siblings.json` ratchet** — done; `allow` array is empty. All apps/web hooks have matching `*.test.ts`/`*.test.tsx` siblings.
- [x] **Promote nursery rules `warn` → `error`** — done; `useExhaustiveSwitchCases` and `useSortedClasses` are at `error` in `biome.json`. `useExplicitType` was dropped (commit `16f2942`, 2026-04-21) as too noisy for incremental benefit.
- [x] **Vitest 4 upgrade pass** — done. Catalog on `vitest@^4.1.5` + `@vitest/browser@^4.1.5`, `vitest-browser-react@2.2.0`, `@vitest/browser-playwright@^4.1.5`. Config uses v4 API (`test.projects`, `provider: playwright()`, `browser.instances`). 51/51 web unit+storybook tests green. Audit turned up zero v3-era patterns (no `environmentMatchGlobs`, no `browser.name`, no `vi.fn<[Args],Return>` generics) — the upgrade had already landed ahead of this TODO.
