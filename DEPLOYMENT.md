...
3. **Use `pnpm exec prisma`, not `npx prisma`.** `npx` resolves packages over the network and fails with `EAI_AGAIN` on locked-down internal networks (air-gapped prod, Kubernetes with restrictive egress). Even when it works it adds cold-start latency. The `migrate` sidecar calls `pnpm --filter @project/db exec prisma db push`. (Prisma 7 removed `--skip-generate`; `db push` no longer invokes generate implicitly.)
